 ...
 * methods to process deprecation information, hints, validation constraints, and OpenAPI
 * annotations during JSON schema generation. This class handles the conversion of Java
 * annotations and metadata into corresponding JSON schema elements.
 */
@AllArgsConstructor
@Slf4j
public class JsonSchemaBuilderHelper {
	...
	/**
	 * Processes deprecation information for a property and updates the JSON schema
	 * properties accordingly. If the property is marked as deprecated, it sets the
	 * deprecated flag and includes detailed deprecation information if available.
	 * @param jsonSchemaProperties The JSON schema properties to be updated with
	...
	/**
	 * Processes hints for a property and updates the JSON schema properties with
	 * examples. If the property has hint values, they are added as examples in the JSON
	 * schema.
	...
	void processHints(JsonSchemaProperties jsonSchemaProperties, Property prop) {
		...
		if (prop.getHint().getValues() != null && !prop.getHint().getValues().isEmpty()) {
			var hints = prop.getHint().getValues().stream().map(HintValue::getValue).filter(Objects::nonNull).toList();
			if (hints.size() > 1) {
	...
	/**
	 * Processes validation annotations on a field and updates the JSON schema properties
	 * accordingly. Handles Jakarta validation annotations such
	 * as @Min, @Max, @Size, @Pattern, @NotEmpty, and @Email.
	 * @param jsonSchemaProperties The JSON schema properties to update with validation
	 * constraints
	...
	public void processValidated(JsonSchemaProperties jsonSchemaProperties, Field field, String propName) {
		...
		if (field.isAnnotationPresent(Size.class)) {
			...
			if (size.min() > 0) {
				...
				log.info("Validation: Added Size.min validation with value {} for field {}", size.min(),
						field.getName());
			}
			if (size.max() < Integer.MAX_VALUE) {
				jsonSchemaProperties.setMaxLength(size.max());
				log.info("Validation: Added Size.max validation with value {} for field {}", size.max(),
						field.getName());
			}
		}
		...
		// log.debug("Validation: Added NotNull validation for field {}",
		// field.getName());
		// }
	 ...
	 * Handles various Schema annotation attributes like description, format, example,
	 * etc.
	 * @param jsonSchemaProperties The JSON schema properties to update with OpenAPI
	 * information
	...
	/**
	 * Processes OpenAPI annotations on a class level and updates the JSON schema
	 * properties. Currently handles the Schema description annotation at class level.
	 * @param jsonSchemaProperties The JSON schema properties to update with class-level
	 * OpenAPI information
