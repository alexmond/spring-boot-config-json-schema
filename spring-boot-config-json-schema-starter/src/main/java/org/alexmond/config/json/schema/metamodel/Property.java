...
/**
 * Represents a configuration property with metadata. This class holds information about
 * configuration properties including their name, type, description, default values, and
 * deprecation status.
 *
 * @since 1.0
 */
...
public class Property {
	...
	/**
	 * Merges this property with a Group, inheriting the group's properties if they are
	 * not empty. Sets this property as a group property and updates name, type,
	 * description, and sourceType from the provided group if they have values.
	...
	public void mergeGroup(Group group) {
		...
		if (StringUtils.hasLength(group.getType())) {
			this.type = group.getType();
		}
		if (StringUtils.hasLength(group.getDescription())) {
			this.description = group.getDescription();
		}
		if (StringUtils.hasLength(group.getSourceType())) {
			this.sourceType = group.getSourceType();
		}
	}
	...
	public void mergeProperties(Property other) {
		if (StringUtils.hasLength(other.getName())) {
			this.name = other.getName();
		}
		if (StringUtils.hasLength(other.getType())) {
			this.type = other.getType();
		}
		if (StringUtils.hasLength(other.getDescription())) {
			this.description = other.getDescription();
		}
		if (StringUtils.hasLength(other.getSourceType())) {
		...
		if (other.getDeprecation() != null) {
			this.deprecation = other.getDeprecation();
		}
