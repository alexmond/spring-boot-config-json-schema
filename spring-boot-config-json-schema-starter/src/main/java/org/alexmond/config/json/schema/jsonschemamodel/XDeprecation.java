...
/**
 * Represents deprecation information for configuration properties. This class holds
 * metadata about deprecated properties including the reason for deprecation, replacement
 * property, deprecation date, and severity level.
 */
@Data
@Builder
@NoArgsConstructor
...
public class XDeprecation {
	...
	public boolean isEmpty() {
		return (!StringUtils.hasLength(reason)) && (!StringUtils.hasLength(replacement))
				&& (!StringUtils.hasLength(since)) && (!StringUtils.hasLength(level));
	}
