...
public class JsonSchemaService {
	...
	/**
	 * Generates a complete JSON Schema representation of the application's configuration
	 * properties.
	...
	/**
	 * Generates a complete JSON Schema representation of the application's configuration
	 * properties and converts it to YAML format.
	 ...
	 * This method implements the core schema generation logic and supports both JSON and
	 * YAML output formats. The generated schema is cached to avoid redundant generation
	 * of the same schema.
	 * @param mapper The ObjectMapper instance to use for serialization (can be configured
	 * for JSON or YAML output)
	 * @return A string containing the schema in the format determined by the provided
	 * ObjectMapper
	 */
	...
	/**
	 * Collects metadata from all spring-configuration-metadata.json files found in the
	 * classpath. This method scans for configuration metadata files and merges their
	 * contents into a single property map.
	...
	public Map<String, Property> collectMetadata() {
		...
		try {
			...
			log.error("Failed to open resources with error {}", ex.getMessage());
